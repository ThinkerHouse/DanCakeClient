...
const PurchaseOrderForm = (props) => {
	const { initialValues, onSubmit, disableCheck, userList, materialsList } = props;
		...
		<AdminForm formik={formik} onBtnCancelClick={props.onCancel} hideSave={disableCheck}>
			<div className="p-6.5">
				...
				<div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
					...
					<div className="w-full xl:w-1/2">
						<FormField type="date" label={"Delivery Date"} id={"delivered_at"} formik={formik} disabled={disableCheck ? true : false} />
					</div>
				</div>
				...
				<div className="mt-15">
					<SectionHeader title={'Purchase Order Items'} buttonText={disableCheck ? '' : 'Add Item'} onButtonClick={handleRowAdd} />
